...
Stylezam AI is separate: each garment has a bounded, locally persisted conversation. On-device token embeddings retrieve relevant structured garment metadata across the Library first; optional Supabase pgvector results can contribute cloud metadata. A turn sends the selected crop, recent conversation context, and at most two additional locally available relevant crops to Fireworks Qwen 3.7 Plus. It never enumerates or uploads the complete Library. Spoken input uses Apple's on-device recognition requirement, keeps audio transient, and inserts only editable text into the composer. Qwen returns both the response and relevant follow-up questions. Only when the user explicitly taps **Find similar** or **Find cheaper** does Stylezam ask Qwen for grounded shopping keywords that can coordinate with the retrieved owned pieces, then sends one generated text query—not a private Library URL—to the next eligible Serper, SearchAPI.io, SerpApi, or Bright Data keyword route. Cheaper-result presentation orders comparable priced results from lower to higher and leaves products without a parsed price afterward.

## Optional private cloud boundary

Supabase is an opt-in synchronization layer, not the detector and not the shopping agent. Firebase remains the identity provider and supplies a third-party JWT. Postgres tables use owner-only RLS, the Storage bucket is private, and metadata embeddings are 256-dimensional deterministic on-device vectors. The sync projection contains accepted garment crops, wardrobe display crops, searches, saved public product metadata, and chat text. It structurally excludes original capture media, reusable person photos, worn-garment reference frames, generated try-on images, and videos. Cloud retrieval returns at most a few row summaries; Fireworks receives only the bounded context selected for the current question.

StoreKit 2 is the source of paid entitlement state on device. The client displays App Store localized prices and verifies transactions before changing its local plan. Production cloud-quota elevation requires server verification and a service-role update to `stylezam_cloud_entitlements`; the publishable iOS client cannot write that table.
