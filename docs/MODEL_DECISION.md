...
- Live camera preview stays single-pass, backs an unchanged empty scene off to a 2.4-second retry cadence, and suppresses repeated inference after an unchanged view is saved. Motion or a detected candidate restores fast confirmation immediately. Live Screen runs a single global tensor immediately for newly sampled content, reserves crop-free global-plus-detail discovery for a matching paused frame whose quick pass was empty, and then uses a focused confirmation. Two agreeing detections trigger the accepted full-quality frame. Captured and known-empty pages fall to a four-second nominal watch cadence with no Core ML until their content changes. Low Power Mode disables detail tiles, fair thermal state slows cadence, serious pressure reduces inference to a cooling cadence, and critical pressure pauses automatic inference.
