...
| Crop validation and visible attributes | Qwen3.7 Plus through Fireworks | Fireworks serverless | One call per look, app-side monthly cap |
| Model/package delivery and request controls | Stylezam FastAPI service | Daytona CPU sandbox | 2 CPU, 4 GB, 10 GB, no GPU |

There is no direct Alibaba/DashScope route, OpenAI route, local model server, or GPU inference route in the active capture pipeline. Qwen3.7 Plus is accessed only through the backend’s Fireworks account.
...
## Why Qwen3.7 Plus on Fireworks

Fireworks lists `accounts/fireworks/models/qwen3p7-plus` as its Qwen3.7 Plus serverless model with text and image input, hosted by Fireworks rather than proxied to Alibaba. Its official launch example uses the OpenAI-compatible chat-completions interface used by Stylezam: [Qwen3.7 Plus on Fireworks](https://fireworks.ai/blog/qwen-3p7-plus).

Stylezam sends one request containing all candidate crops and a JSON Schema. The request uses temperature zero, `reasoning_effort: "none"`, and a bounded output-token count. Qwen3.7 Plus may:
...
Qwen3.7 Plus does not decide the mask and does not search for products in this release.

Fireworks charges serverless model use according to the active model pricing. The default `STYLEZAM_FIREWORKS_MONTHLY_CAP=100` blocks the 101st Stylezam request in a UTC month, but the first 100 remain billable according to the user’s Fireworks account. This application cap is not an account billing control. Fireworks separately documents prepaid credits and a configurable monthly spend limit that pauses all account API requests when reached; Stylezam’s chosen account-wide limit is $50: `firectl quota update monthly-spend-usd --value 50`. Confirm it with `firectl quota list` before production use: [Fireworks account quotas](https://docs.fireworks.ai/guides/quotas_usage/account-quotas).
...
| Direct Alibaba/DashScope Qwen API | Not used; Qwen3.7 Plus runs through Fireworks with the same server-only Fireworks credential. |
| MiniMax M3 | Not selected after the Qwen3.7 Plus provider decision. |
