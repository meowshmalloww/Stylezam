...
The app separates YouCam inputs into Outfit, Head, Hand/Wrist, and Face/Neck photo contexts because accessories and jewelry require closer framing than clothing. It infers the context from the selected pieces and person photo, then keeps the manual picker available as a correction. An Outfit photo can still apply a necklace with the clothing look. The Automatic/Female/Male presentation control is available for Outfit and the Hat, Bag, Scarf, and Shoes endpoints that require it. Selected pieces that do not match the active context stay parked and are excluded before any paid request. The Try On workspace opens its in-app camera front-facing, shows a transparent three-second countdown before capture, supports rear/front switching, pinch and preset zoom, or Photos import. Person photos persist locally and can be reused by swiping. The expandable Pieces/Shop rail persists selection state and available merchant links; removing a rail entry does not delete its wardrobe item.

Images are normalized to YouCam's supported bounds and remain under 10 MB. The UI verifies basic credential connectivity, requires explicit upload consent, shows the real request phase and exact task count, and explains YouCam's documented retention boundary. Every selected category uses the documented shared File API, then its own category task endpoint. One Create action applies compatible selected items sequentially through category-specific tasks, using each result as the source for the next item; it is not a one-call multi-garment operation. Clothes use V4, including `change_shoes = false` when shoes are not the selected category. Product references for Hat, Bag, Scarf, and Shoes should show exactly one unobstructed product that fills at least one quarter of the image height. Lower-body clothing references must visibly show the garment being worn by one clear person rather than as a standalone product image. For a detected lower-body piece, Stylezam stores the full originating frame as a separate durable, best-effort reference candidate while continuing to show its tight crop in the rail. Strong bedding/furniture references and obvious full-scene replacement outputs are blocked locally. The provider’s Hat, Bag, Scarf, and Shoes APIs are style-generating endpoints, so Stylezam refuses rather than saves a result that visibly rewrites the person or background; this is a safety boundary, not a claim that every provider output is pixel-preserving. The rail reports missing state, blocks Create when needed, and lets the user replace the reference from Photos. Identical full-frame bytes share one content-addressed file. Optional post-processing can apply enhancement, lighting, background removal, or background replacement. **View as video** requests a five-second YouCam image-to-video v2 result at 480p, 720p, or 1080p and provides a replay control that does not cover the generated image. Saving records the still plus applied and parked item manifests and their available purchase links.
