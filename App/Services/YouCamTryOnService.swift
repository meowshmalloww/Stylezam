...
enum YouCamCredentialStore {
    ...
    static var isConfigured: Bool {
        ...
        if let value = Bundle.main.object(forInfoDictionaryKey: "YouCamAPIKey") as? String,
           !value.isEmpty, !value.contains("$(") { return true }
        return false
    }
...
enum YouCamTryOnError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingAPIKey:
            "Enter a YouCam API key in the Try On connection panel, or configure YOUCAM_API_KEY for this build."
        case .invalidResponse:
...
actor YouCamTryOnService {
    ...
    func render(
        ...
        for (index, item) in items.enumerated() {
            ...
            let referenceID = try await upload(item.imageData)
    ...
    private func createTask(
        ...
        if category.isJewelry {
            ...
            body["object_infos"] = [["name": referenceID]]
        } else {
    ...
    private static var apiKey: String? {
        ...
        return Bundle.main.object(forInfoDictionaryKey: "YouCamAPIKey") as? String
    }
